...
The first ACP implementation is local subprocess over stdio. This transport still permits local multi-agent coordination through concurrent sessions in a capable runtime and/or multiple explicitly started subprocesses. HTTP/WebSocket ACP is deferred for remote or independently scaled runtime pools; transport does not own delegation, dependencies, evidence, budgets, or acceptance.
...
- Whether later releases add configurable expiry for normalized closed-session metadata; the v1 contract clears resumable opaque references on close/archive and otherwise retains bounded metadata with the workspace.
