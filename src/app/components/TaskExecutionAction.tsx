        ...
        >
            ...
            {binding && (
              <div className="rounded-md border border-slate-200 p-3">
                <div className="grid gap-2 sm:grid-cols-2">
                  <div className={`rounded-lg p-3 ${isTurnActive ? 'bg-emerald-50' : 'bg-slate-50'}`}><div className="flex items-center gap-2 text-xs font-semibold text-slate-500"><span className={`size-2 rounded-full ${isTurnActive ? 'bg-emerald-500' : 'bg-slate-400'}`} />Agent status</div><div className="mt-2 font-semibold text-slate-900">{sessionSummary?.label}</div><div className="mt-1 text-xs leading-5 text-slate-600">{sessionSummary?.detail}</div></div>
          ...
          <SheetFooter className="sticky bottom-0 z-10 mt-0 shrink-0 border-t border-black/6 bg-white px-6 py-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex min-w-0 flex-1 items-start gap-2 text-left text-xs text-slate-500">
              ...
              {binding
                ? isTurnActive ? 'Codex is working on this task now.' : binding.state === 'interrupted' ? 'Work was interrupted. Resume to continue with the task instructions.' : binding.state === 'failed' ? 'Work stopped because the runtime reported an error.' : 'The latest run is not active. Follow-up guidance is optional.'
                : activeAttempt
            ...
            <div className="flex shrink-0 justify-end gap-2">
              ...
              <button type="button" onClick={() => void (binding?.state === 'interrupted' ? resumeSession() : startWork())} disabled={operationBusy || (binding?.state === 'interrupted' ? !resolvedRepositoryFolder : loading || blockers.length > 0 || Boolean(binding) || activeAttempt)} className="rounded-md bg-slate-900 px-3 py-2 text-sm font-semibold text-white disabled:opacity-40" title={isTurnActive ? 'Codex is working' : undefined}>{binding?.state === 'interrupted' ? 'Resume work' : binding ? (isTurnActive ? 'Working' : 'Work session open') : activeAttempt ? 'Execution active' : operationBusy ? 'Starting…' : 'Start work'}</button>
            </div>
          </SheetFooter>
        </SheetContent>
      </Sheet>
