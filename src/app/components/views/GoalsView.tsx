...
import { DeleteGoalDialog, NewGoalDialog, ResetGoalExecutionDialog } from '../goals/GoalsDialogs';
...
function goalExecutionAttention(state: string): { label: string; description: string; nextStep: string } {
  if (state === 'working') return { label: 'Active execution', description: 'An agent is currently executing this Goal.', nextStep: 'Open the selected node in the inspector to monitor its work.' };
  if (state === 'blocked') return { label: 'Blocked execution', description: 'Execution cannot continue until its blocking reason is resolved.', nextStep: 'Review the selected node and resolve its dependency, policy, or agent blocker.' };
  if (state === 'failed') return { label: 'Failed execution', description: 'The last execution failed and requires review before retrying.', nextStep: 'Review the runtime details, then reset execution before retrying.' };
  if (state === 'approval-required') return { label: 'Approval required', description: 'Execution is waiting for a configured approval decision.', nextStep: 'Review the approval gate and record the decision.' };
  if (state === 'evidence-required') return { label: 'Evidence required', description: 'Execution is waiting for evidence before handoff.', nextStep: 'Attach or verify the required evidence in the inspector.' };
  if (state === 'permission-denied') return { label: 'Permission denied', description: 'The current permissions do not allow execution to continue.', nextStep: 'Review the policy or ask an administrator for the required permission.' };
  if (state === 'handoff-pending') return { label: 'Pending human review', description: 'Execution has produced a handoff that needs human attention.', nextStep: 'Review the evidence and accept or reject the handoff.' };
  if (state === 'complete') return { label: 'Execution complete', description: 'This execution fulfilled its contract.', nextStep: 'No action is pending; reset execution only if you want to run it again.' };
  return { label: state === 'ready' ? 'Ready to execute' : state.replaceAll('-', ' '), description: 'The runtime has recorded this execution state for the active Goal revision.', nextStep: 'Review the selected node in the inspector for available actions.' };
}
...
export function GoalsView({ people = [], tasks = [], milestones = [], projects = [], workspacePolicy, goalAuditArchiveDirectory = '', onGoalAuditArchiveDirectoryChange }: { people?: Person[]; tasks?: Task[]; milestones?: ProjectMilestone[]; projects?: TimelineSwimlane[]; workspacePolicy?: GoalPolicyV1; goalAuditArchiveDirectory?: string; onGoalAuditArchiveDirectoryChange?: (directory: string) => void }) {
  ...
  const [resetExecutionDialogOpen, setResetExecutionDialogOpen] = useState(false);
  ...
  const resetGoalExecution = async () => {
    ...
    setResetExecutionDialogOpen(false);
  ...
  <section className="goals-view relative h-full min-h-0 overflow-hidden bg-slate-50 text-slate-700">
    ...
    <GoalsCanvasSurface canvasRef={canvasRef} spacePressed={spacePressed} panMode={panMode} pan={pan} zoom={zoom} emptyState={!activeGoal ? <GoalsCanvasEmptyState onNewGoal={openNewGoalDialog} /> : undefined} onPointerDown={beginCanvasPan} onPointerMove={moveCanvasPan} onPointerUp={endCanvasPan} onPointerCancel={endCanvasPan} onLostPointerCapture={event => { if (panSessionRef.current?.pointerId === event.pointerId) panSessionRef.current = null; }}>
        {runtimeProjection?.execution?.state && (() => { const attention = goalExecutionAttention(runtimeProjection.execution.state); return <div role="status" className="absolute left-4 top-4 z-10 max-w-[min(520px,calc(100%-2rem))] rounded-md border border-blue-200 bg-white/95 px-3 py-2 text-xs text-slate-600 shadow-sm"><div className="flex items-start gap-3"><span className="min-w-0"><span className="font-semibold text-slate-800">{attention.label}</span><span className="mt-0.5 block text-[11px] text-slate-500">{attention.description}</span>{runtimeProjection.execution.state === 'working' && runtimeProjection.execution.workerDelegationStatus === 'required' && <span className="mt-1 block text-[11px] text-amber-700">Subagent recruitment required before dependent work proceeds.</span>}<span className="mt-1 block text-[11px] text-slate-600"><span className="font-semibold text-slate-700">Next step:</span> {attention.nextStep}</span></span><button type="button" onClick={() => setResetExecutionDialogOpen(true)} className="shrink-0 rounded-md border border-slate-200 bg-white px-2 py-1 text-[11px] font-semibold text-slate-600 hover:border-slate-300 hover:bg-slate-50">Reset execution</button></div></div>; })()}
    ...
    {selectedElement && (
      <GoalsInspector selectedElement={selectedElement} selectedElementLocked={selectedElementLocked} onDelete={deleteElement} goalCopyState={goalCopyState} onCopyGoalDetails={() => { void copyGoalDetails(); }}>
        ...
        {selectedElement.type === 'goal' && <GoalsScheduleSection schedule={activeSchedule} latestOccurrence={runtimeProjection?.scheduleOccurrences?.slice().reverse().find(occurrence => occurrence.scheduleId === activeSchedule?.id)} onCreate={createSchedule} onUpdate={updateSchedule} onDelete={deleteSchedule} />}
        ...
        <GoalsRuntimeStatus element={selectedElement} connected={isGoalElementConnected(activeGoal?.elements ?? [], selectedElement.id)} runtimeProjection={runtimeProjection} onResetExecution={() => setResetExecutionDialogOpen(true)} />
    ...
    <ResetGoalExecutionDialog open={resetExecutionDialogOpen} onCancel={() => setResetExecutionDialogOpen(false)} onConfirm={() => { void resetGoalExecution(); }} />
