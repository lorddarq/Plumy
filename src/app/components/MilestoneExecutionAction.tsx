      ...
      <DialogContent className="z-[100] max-w-2xl">
        ...
        <div className="space-y-2 text-sm">
          ...
          {rows.map(row => <div key={row.task.id} className="rounded-md border border-slate-200 p-3">
            <div className="flex items-start gap-2"><div className="min-w-0 flex-1"><div className="font-medium text-slate-900">{row.task.title}</div><div className="mt-1 flex flex-wrap gap-3 text-xs text-slate-500"><span className="inline-flex items-center gap-1"><Server className="size-3" />{row.runtime}{row.sessionState ? ` · ${row.sessionState}` : ''}</span><span className="inline-flex items-center gap-1"><Folder className="size-3" />{row.folderSource}: {row.folder || 'Unavailable'}</span>{row.accepted > 0 && <span>{row.accepted} accepted contribution{row.accepted === 1 ? '' : 's'}</span>}{row.usage && <span>Usage {row.usage}</span>}</div></div>{row.active ? <span className="text-xs font-semibold text-blue-700" aria-label={row.sessionState === 'needs-input' ? getAttentionState('needs-input').label : getAttentionState('active').label}><span aria-hidden="true" className="mr-1">{row.sessionState === 'needs-input' ? getAttentionState('needs-input').symbol : getAttentionState('active').symbol}</span>{row.sessionState === 'needs-input' ? getAttentionState('needs-input').label : getAttentionState('active').label}</span> : row.blockers.length ? <span className="inline-flex items-center gap-1 text-xs font-semibold text-red-700" aria-label={`${getAttentionState('blocked').label}. ${getAttentionState('blocked').nextStep}`}><span aria-hidden="true">{getAttentionState('blocked').symbol}</span>{getAttentionState('blocked').label}</span> : <span className="inline-flex items-center gap-1 text-xs font-semibold text-emerald-700" aria-label={`${getAttentionState('ready').label}. ${getAttentionState('ready').nextStep}`}><span aria-hidden="true">{getAttentionState('ready').symbol}</span>{getAttentionState('ready').label}</span>}</div>
