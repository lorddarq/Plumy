    ...
    <div className="space-y-8">
      ...
      <section className="space-y-4" aria-labelledby="runtime-profiles-title">
        ...
        <div className="grid gap-3 rounded-xl border border-[#ececf0] p-4 sm:grid-cols-2">
          ...
          {draft.integrationMode === 'claude-stream-json-stdio' && <label className="space-y-1 sm:col-span-2"><span className="text-xs font-semibold text-[#71717a]">Claude permission mode</span><select value={draft.permissionMode || ''} onChange={event => setDraft({ ...draft, permissionMode: (event.target.value || undefined) as AgentRuntimeProfile['permissionMode'] })} className={`${FIELD_CLASS} w-full`}><option value="">Use Claude default</option><option value="default">Default</option><option value="acceptEdits">Accept edits</option><option value="plan">Plan only</option><option value="auto">Auto</option><option value="dontAsk">Don't ask (deny unapproved tools)</option><option value="bypassPermissions">Bypass permissions</option></select><span className="block text-xs leading-5 text-[#7f8796]">Uses Claude's native --permission-mode flag. Omvra lifecycle approval gates remain separate.</span>{draft.permissionMode === 'bypassPermissions' && <span className="block rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-xs leading-5 text-red-800">Bypass permissions disables Claude's safety checks. Use only in an isolated workspace.</span>}</label>}
